...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const decision = await authorize({ userId: user.id, permission: "privacy.resolve", resource: { organizationId: resource.organizationId, siteId: resource.siteId, serviceKey: resource.sourceKind, researchUse: resource.researchUseStatus } });
