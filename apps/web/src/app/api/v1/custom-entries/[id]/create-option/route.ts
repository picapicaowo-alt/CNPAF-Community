...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try { return NextResponse.json(await reviewCustomEntry({ id, actorId: user.id, action: "created_new", body: parsed.data })); }
  catch (error) { const message = error instanceof Error ? error.message : "Could not create option"; return jsonError(message, message === "Forbidden" ? 403 : 409); }
}
