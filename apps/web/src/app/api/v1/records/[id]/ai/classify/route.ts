...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const decision = await authorize({ userId: user.id, permission: "ai.request_reclassification", resource: { organizationId: row.record.organizationId, siteId: row.record.siteId, serviceKey: row.record.sourceKind, ownerUserId: row.record.createdById } });
