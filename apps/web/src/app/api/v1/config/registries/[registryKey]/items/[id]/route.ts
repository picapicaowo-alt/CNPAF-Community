...
export async function PATCH(req: Request, { params }: { params: Promise<{ registryKey: string; id: string }> }) {
  ...
  try {
    const item = await updateRegistryItem(id, parsed.data, user.id);
    await audit({ actorId: user.id, action: "registry.item_updated", entityType: "config_registry_item", entityId: id, afterState: item });
    return NextResponse.json({ item });
  } catch (error) {
    return jsonError(error instanceof Error ? error.message : "Could not update registry item", 409);
  }
}
